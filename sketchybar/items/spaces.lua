...
local all_workspaces = exec_lines("/opt/homebrew/bin/aerospace list-workspaces --all")

for _, sid in ipairs(all_workspaces) do
	-- Determine which monitor this workspace is on
	local monitor_id = exec(
		string.format(
			"/opt/homebrew/bin/aerospace list-windows --workspace %s --format '%%{monitor-appkit-nsscreen-screens-id}' | cut -c1",
			sid
		)
	)
	if monitor_id == "" then
...
local function build_current_state()
	local state = {}
	local focused_workspace = exec("/opt/homebrew/bin/aerospace list-workspaces --focused")

	-- Only build state for non-empty workspaces
	local monitors = exec_lines("/opt/homebrew/bin/aerospace list-monitors")
	for i, _ in ipairs(monitors) do
		local non_empty_workspaces = exec_lines("/opt/homebrew/bin/aerospace list-workspaces --monitor " .. i .. " --empty no")

		for _, sid in ipairs(non_empty_workspaces) do
			local apps_raw = exec(string.format("/opt/homebrew/bin/aerospace list-windows --workspace %s", sid))
...
local function update_all_workspaces()
	...
	local empty_workspaces = exec_lines("/opt/homebrew/bin/aerospace list-workspaces --all --empty yes")
	for _, sid in ipairs(empty_workspaces) do
		-- WARN: this assumes monitor 1 is your main monitor
		os.execute(string.format("/opt/homebrew/bin/aerospace move-workspace-to-monitor --workspace %s 1", sid))
	end
